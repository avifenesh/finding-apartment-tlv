...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setInterval(() => {
        if (!isValidToken(sessionStorage.getItem('authToken'))) {
            logout();
        }
    }, 60000);
});
...
function createApartmentCard(apartment) {
    ...
    card.innerHTML = `
        ...
        <div class="p-4">
            ...
            <a href="${apartment.link}" target="__blank" class="block w-full text-center bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                צפה במודעה ב-Yad2
            </a>
        </div>
    `;
